...
const FLIGHT_TERMINAL = new Set(["Landed", "Cancelled"]);
...
export default function BookingDetail() {
  ...
  const handleForceRefreshFlight = async () => {
    if (!booking?.flight_number || !booking?.date_time) return;
    setFlightRefreshing(true);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      const token = session?.access_token ?? "";
      const flightDate = new Date(booking.date_time).toISOString().split("T")[0];
      const dir = (booking as any).direction === "Departure" ? "Departure" : "Arrival";
      const fn  = booking.flight_number.replace(/\s+/g, "").toUpperCase();
      const res = await fetch(
        `/api/flight-tracker/${encodeURIComponent(fn)}/force-refresh?date=${flightDate}&direction=${dir}`,
      ...
      if (res.ok) {
        ...
        toast({ title: "Refresh failed", description: body.error ?? "Unknown error", variant: "destructive" });
      }
    } catch (e: any) {
      toast({ title: "Refresh failed", description: e?.message ?? "Network error", variant: "destructive" });
    } finally {
  ...
  return (
    <div className="space-y-3 max-w-3xl mx-auto pb-10">
      ...
      {booking.flight_status && (() => {
        ...
        // Stale-data guard: "Early" or "Landed" but the estimated arrival time
        // is still in the future — AeroDataBox returned bad data before the plane
        // actually landed. Show a warning and let the user force-refresh.
        const estimatedMs = fs.estimated_time ? new Date(fs.estimated_time).getTime() : null;
        const isStale = (fs.status === "Early" || fs.status === "Landed")
          && estimatedMs !== null
          && estimatedMs > Date.now();

        return (
          <Card className={`border-blue-500/30 bg-blue-500/5 ${isStale ? "border-amber-500/50 bg-amber-500/5" : ""}`}>
            <CardContent className="p-4">
              {isStale && (
                <div className="flex items-center gap-2 mb-3 text-xs text-amber-400 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2">
                  <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0" />
                  <span>Status may be incorrect — flight data was cached before the plane arrived. Tap Refresh to fetch the latest.</span>
                </div>
              )}
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right space-y-1 flex-shrink-0">
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1">
                    ...
                    <button
                      type="button"
                      onClick={handleForceRefreshFlight}
                      ...
                      title="Force-refresh flight status from AeroDataBox"
                    >
                      <RotateCcw className={`w-3 h-3 ${flightRefreshing ? "animate-spin" : ""}`} />
                      {flightRefreshing ? "Refreshing…" : "Refresh"}
                    </button>
                  </div>
                </div>
              </div>
