            ...
            <CardContent className="p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="font-bold text-lg text-foreground flex items-center gap-2">
                    ...
                    <Badge variant="outline" className={flightStatusColor(tracker.flight_status?.status)}>
                      {tracker.flight_status?.status || 'Unknown'}
                    </Badge>
                  </h3>
                  <div className="text-sm text-muted-foreground">
                    {tracker.flight_status?.origin || 'Unknown'} → {tracker.flight_status?.destination || 'Unknown'}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full md:w-auto text-sm">
                <div>
                  ...
                  <span className="font-medium text-foreground">{tracker.scheduled_time || '-'}</span>
                </div>
                <div>
                  <span className="block text-xs uppercase text-muted-foreground opacity-70">Estimated</span>
                  <span className="font-medium text-foreground">{tracker.flight_status?.estimated_time || '-'}</span>
                </div>
                <div>
