...
export default function Clients() {
  ...
  const handleBulkDelete = async () => {
    ...
    toast({
      ...
      description: (failed === 0 ? `${deleted} client${deleted === 1 ? "" : "s"} permanently removed` : "Some deletions failed — check audit log") + missedNote,
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        ...
        if (nationalityFilter) {
          const { flag } = detectNat(null, null, nationalityFilter);
          const natLabel = (flag && flag !== "🌍") ? `${flag} ${nationalityFilter}` : nationalityFilter;
