...
router.post("/bulk-delete", async (req, res) => {
  ...
  // Pre-check which request IDs actually exist so we can report missing accurately.
  const { data: found } = await supabase
    .from("requests").select("id").in("id", cleanIds);
  ...
  const { error } = await supabase.from("requests").delete().in("id", foundIds);
  if (error) return res.status(400).json({ error: error.message });

  await auditLog(
    "bulk_delete_requests", "request", foundIds[0], user.id,
    `Bulk deleted ${foundIds.length} request(s) by ${user.name ?? user.email ?? user.id}`,
  );
