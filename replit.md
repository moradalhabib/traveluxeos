...
- **Cancelled-by attribution (May 2026)** — Both the request cancellation banner on `/requests/:id` and the cancelled card row on `/follow-ups` show a single-line `Cancelled by <Name> · <d MMM HH:mm> · Reason: <reason>` audit line. The actor name is wrapped in the existing tooltip primitive showing their email so two-admin teams can disambiguate when display names collide. Server enrichment: `GET /api/requests/:id` and `GET /api/follow-ups` join `users` on `cancelled_by` and return `cancelled_by_name` + `cancelled_by_email`. Soft-deleted users already have `name="[removed]"` and a safe email placeholder (`routes/users.ts`), so original emails of removed staff are never leaked. Falls back to plain "Cancelled · …" if the actor row is missing (legacy rows from before `cancelled_by` existed).
